...
async def fetch_pelando_deals() -> list[Deal]:
    ...
    async with httpx.AsyncClient(timeout=15) as client:
        for feed_url in PELANDO_RSS_FEEDS:
            try:
                # [FIX] feedparser.parse() é síncrona e bloqueante.
                # asyncio.to_thread() a delega para uma thread do pool,
                # liberando o event loop enquanto aguarda a resposta HTTP.
...
async def fetch_custom_rss() -> list[Deal]:
    ...
    for feed_url in CUSTOM_RSS_FEEDS:
        try:
            # [FIX] Mesmo fix: to_thread para não bloquear o event loop
            feed = await asyncio.to_thread(feedparser.parse, feed_url)
